...
@router.delete("/comandas/{comanda_id}", tags=["Comanda"])
async def delete_comanda(comanda_id: int):
    try:
        session = db.Session()
        # busca a comanda
        dados = session.query(ComandaDB).filter(ComandaDB.id_comanda == comanda_id).one()
        # exclui
        session.delete(dados)
        session.commit()
        return {"id": dados.id_comanda}, 200
